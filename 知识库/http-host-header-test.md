...
> **实战案例（双向引用）** → `logic-web-cases.md` §7.1（**改 Host 即通杀约 15 个系统**的未授权添加管理员框架，删 Cookie 仍可匿名加管理员）；`attack-chain-cases.md` §6.1（**Host 头碰撞**：`req.Host=domain` 打内网隐藏靶标，工具 host_scan/Hosts_scan）+ §4.1（**AC 设备按 Host 白名单放行** → 把 CS listener 的 HTTP Host Header 配成目标官网域名即上线）；`ssrf-cases.md` §四（Host 头 2 份 blind）；`idor-cases.md` §3.10（重置链接中的 Host 与账号接管）。
