...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无 GraphQL 报告**。相邻可用的「拿全量接口」思路：`code-audit-cases.md` §三（抓 `swagger.json` → AppSpider 半自动扫 → 手工 `PUT` 改字段验注入/XSS）；`infoleak-cases.md` §2（**Swagger 被删/鉴权时走 `/actuator/mappings` 取 `patterns` 拿全量路由**，可替代 GraphQL introspection）；`middleware-unauth-test.md`（未授权 Swagger/Actuator 面）。
