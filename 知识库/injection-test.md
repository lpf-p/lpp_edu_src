...
> **实战案例（双向引用）** → `sqli-cases.md`（SQL 注入 97 条）与 `cmd-injection-cases.md`（命令注入/RCE 49 条 → 38 唯一 → 实读 25，含组件指纹速查表、不出网无回显落地）；回查原文见 `ima-retrieval-index.md` §三。
...
已确认**有差分面**但无回显 → 转 `sqli-advanced-test.md`：布尔盲注两条都打、时间盲注基线+3 次、带外 OOB 先测出网、二次注入看**读取步**、堆叠只证明、文件读写只最小。**无差分面不要上重武器。**
...
> **打深篇** → `ssti-test.md`（认渲染口 / 四探针定引擎 / 报错抓版本查 Nday / 各栈沙箱逃逸 / 无回显判定 / 过滤绕过）。本篇只给快速检测，**认到渲染口之后直接翻那篇**。
> 表达式语言（`SpEL` / `OGNL` / `Java EL`）单独成篇 → `el-injection-test.md`，别把 SpEL 当 SSTI 打。
