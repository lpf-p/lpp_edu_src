...
> **实战案例（双向引用）** → ima 案例库 1604 份（多为 2021 年前报告）中**无 AI Agent / 对话口工具执行类报告**，本篇靠打法支撑。相邻：`idor-cases.md`「助手历史未授权读他人任务」（正文只在那一篇）、`mobile-cases.md` §7（第三方与供应链：GitHub 泄露前端仓库 `MAIL_CONFIG` 明文 username/password）、`csrf-cases.md` 案例 #20（百度 AI Studio 改头像/登出 JSON 接口无 token——**这是 CSRF 不是本篇**）。
