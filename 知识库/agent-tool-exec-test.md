...
5. 命令跑起来后跟这三样（有入口就打，没有写原因）：SRC 给的内网验证台 flag；云元数据（**只验证接口可达 / 返回结构，不取凭证值、不落盘、不复用**）；沙箱里他主体业务正文（对话历史、工单、密钥文件）。公网首页不算通内网。⚠️ 云元数据临时钥 = 实质接管云资源，**只在平台明确授权的内网验证环境里做**，边界见 `rules/test-scope-boundary.md` §五  
