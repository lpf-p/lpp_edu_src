...
>
> **多资产 / 模式库全量过 / 别人给的 N 份报告 zip → 先读** `rules/batch-verify-discipline.md`：**基线采样 + 只认结构性证据**两条铁律，以及「N 份 = 1 漏洞 × M 主机」的批量报告鉴定口径。判据细节只在那份 rules，本文件不重复。
>
> **FOFA 在 fofa.info 网页手动跑**（把下面语法直接抄进搜索框）。本技能不捆绑 fofa MCP、不要求 key；案例库每条都带可直接抄的查询串。无配额时走 `dig-scope` §2.1.4 的 **DNS+HTTP+crt 活筛兜底**，不要空刷。**禁止**把任何 email / key 写进本文件或对话。

> **实战案例（双向引用）** → `attack-chain-cases.md` §2.1 目标选取 + §2.2 资产测绘手法清单（源码/地图文件、被动接口扫描、**C 段统计脚本 ≥5 优先打**）；`other-census.md` §6.3（`上海交通大学最全资产.xlsx` `序号|域名|长度|服务|状态|指纹|ip|标题` 全域子域表）+ §三（资产测绘与工具 11 条：`资产发现之垂直/水平关联`、`ARL 大改造`、`FOFA 公a版白嫖`）；`ima-case-corpus.md` §3（**测绘语法定位 → 同套系统批量 → POC 命中** 的通用模板）+ §4.3（报告结构模板）；`edusrc-cases.md` §一（`site:*.edu.cn` + 校名，教育单位子域极多）。
...
> 本节的 whatweb / httpx **只认海外 CMS，认不出国内 SRC 的主流系统**（站点群 / AWS PaaS / 强智 BH / 正方 / 帆软 / 泛微 / 致远 / GVA / 若依…）。
> **国产系统指纹 → 该打什么**的映射表、favicon hash 反查算法见 `知识库/recon-fingerprint-cdn-wildcard.md` §1。
> **CDN 穿透找源站**见同文件 §2，**泛解析判定与子域过滤**见 §3——这两件本节完全没写，别以为扫完本节就够了。
