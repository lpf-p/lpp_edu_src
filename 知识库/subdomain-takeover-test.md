...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无子域接管（CNAME 悬空）报告**。库里的「接管」是别的形态，别混写：`mobile-cases.md` §1（**云存储 AK/SK 与 bucket 接管**：反编译源码搜 `OSS_CONFIG`/`accessKeyId`/`accessKeySecret`/`bucket`，命中即用 OSS 工具接管）；`vendor-system-cases.md` #18（某保险云 `actuator/env` 泄露华为云 OBS AK/SK + OSS bucket → 行云管家导入接管 37 台）；`ssrf-cases.md` §三（SSRF → 云元数据 → 临时 AK/SK → `AssumeRole` 横向 → 接管控制台）；`edusrc-cases.md` §2.1 账号接管类。
