...
> **实战案例（双向引用）** → `file-upload-cases.md`（ima `src报告/文件上传/` 40 条 → 32 份唯一 → 实读 18）：按**防护层级**重组的绕过全景、编辑器/组件上传点速查表；WAF 侧的头字段与文件名绕过另见 `waf-bypass.md` §2.8.1。回查原文见 `ima-retrieval-index.md` §三。
