...
> **实战案例（双向引用）** → `code-audit-cases.md` §1.1 + §二危险函数表（`SpelExpressionParser.parseExpression(el)` 中 `el` 外部可控即 RCE，CVE-2018-1260 同类）；`cmd-injection-cases.md` §二打法分类 2（AJ-Report `validationRules` 内 `new java.lang.ProcessBuilder("whoami").start()`、美团 `groovyInput`）；`attack-chain-cases.md` §3（Struts2 OGNL）。**与 `ssti-test.md` 的分工**：模板**渲染**口走 SSTI，表达式**求值**口走本篇。
