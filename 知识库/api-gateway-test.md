...
> **实战案例（双向引用）** → `mobile-cases.md` §5（**网关不校验完整身份**：小程序只要 `Bearer` token + 自研头 `QI-Client-Id`/`QI-Auth-Sign`/`QI-Auth-Timestamp` 就放行，网关只认头不认人）；`weak-password-cases.md` §一（锐捷锐易网关 `admin:admin`，`:6060`/`:8000`/`:9001`，路径 `/cgi-bin/luci/;stok=.../admin`）；`vendor-system-cases.md` §1（电信网关管理后台默认口令 `admin/hassmedia`）；`other-census.md` §6.2（浙江计量院 `/api/actuator/env` 未授权读出网关路由 / `spring.redis.host` / 数据源）。
