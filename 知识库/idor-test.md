> 写不写只认 `~/.workbuddy-ai/skills/lpp_edu_src/rules/vuln-report-format.md`。进站有会话时最低探针见 `dig-scope` §4.2.3（对象图、换 id、哨兵值）；单号用列表/回包里的他人 id，不为第二号磨注册。默认先用读/列表差分证明跨用户·跨租户。写越权仍要测，但**不是**对着别人已有数据改/删。顺序见下「写越权怎么打」。禁止批量改删、禁止真资损。
> 短表指针用标题搜。英文 BOLA 百科已砍；写越权怎么打仍在上半。
> **实战案例（双向引用）** → `idor-cases.md`（ima `src报告/越权/` 168 条 → 112 份唯一精读）：对象 ID 替换 / 用户标识参数替换（`userId`·`unionId`·`loginUuid`·`memberId`）/ 响应包篡改 / Cookie·token 缺陷 / 删参改排序退化全表 / 路径替换·直访受限 URL / 客户端鉴权 / 垂直越权 / 未授权访问 / 任意密码重置 9 类 / 多租户通杀；附 CNVD 设备指纹速查表与麦当劳 SRC 定级尺度。**教育侧越权见 `edusrc-cases.md` §2.8**（按教育业务场景组织）。
...
**与 SQLi / XSS 的本质区别（决定了怎么找它）**：

```
SQLi / XSS  → 恶意输入 → 触发异常 → 有报错 / 有弹窗 → 靠"看异常"发现
越权         → 合法请求 → 正常执行 → 200 + 数据正常 + 无报错 → 只能靠"差分对比"发现
```

**所以：越权不会自己跳出来，必须主动做差分。** 这是唯一可靠的检测方式，不存在"扫一下就知道"。

**一句话**：**登录 ≠ 授权**。登录验证「你是谁」，授权验证「你能做什么」。越权 = 只做了前半句。

### 五大根因

| 根因 | 典型代码/现象 | 问题 |
|---|---|---|
| **只校验登录，不校验权限** | `@login_required` 但没有角色判断 | 登录了 ≠ 有权限做所有事 |
| **信任前端传参** | `userId = request.form['userId']` | 攻击者能改，后端不该信 |
| **前端隐藏代替后端校验** | 普通用户页面不显示管理按钮 | 接口 URL 仍能直接访问 |
| **自增 ID 可预测** | 订单 1001 → 1002 | 改个数字遍历全量 |
| **开发者惯性思维** | "普通用户怎么会知道管理接口 URL？" | 攻击者一定会枚举 |
...
```
账号 A：普通用户（攻击发起方）
账号 B：普通用户（A 的攻击目标，与 A 同级）
账号 C：低权限 / 新注册用户（测垂直越权）
```

- **没有第二号**：用列表、回包、邻号、`0`/`-1`/空当他人 id（见 Step 2）。**不为凑号磨注册**。
- **能注册高权限就注册**：对比高低权限界面差异 → 抓高权限流量包 → 换 cookie 重放。高权限功能没鉴权 = 垂直越权。
- 至少一个号能登录就够开工，别卡在"没号"。
...
| 现状 | 动作 | 命中含义 |
|---|---|---|
| 请求**根本没有**鉴权字段 | **手动加** `userId=他人ID` | 后端完全不校验归属 |
| 有鉴权字段但**值为空** | 填他人 uid / 管理员 uid | 缺非空校验与归属校验 |
| 有**合法**鉴权值 | 替换为他人凭证（token/cookie） | 会话绑定缺失 |
| 有合法值 + 资源 id | 改资源 id | 水平越权（经典 IDOR） |

> **「加参数」是最常被漏的一枪。** 多数人只测「改」，不测「加」——请求里没有 `userId` 不代表后端不认这个参数。

---

## ID 枚举三法（水平越权核心）

1. **顺序遍历**：Intruder / ffuf 打 1~N，看响应差异
2. **从其他接口泄露收集**（命中率最高）：搜索/列表接口返回的 `userId` 列表，收集后逐个越权
3. **业务推测**：注册时返回 `userId=1088` → 1~1087 大概率都是已有用户

**UUID 不是防护措施**：可能在别的接口响应、JS、前端路由里泄露。加密 ID 同理（那是混淆，不是鉴权）。

---

## 工具（Autorize 是越权自动化首选）

| 工具 | 用途 | 备注 |
|---|---|---|
| **Autorize**（Burp 插件） | 配低权限 Cookie，**自动重放所有请求并标记权限差异** | 越权规模化检测第一选择 |
| **AuthMatrix**（Burp 插件） | 多角色权限矩阵，同时管多个 Cookie 批量测接口权限 | 角色多时用它 |
| Intruder / ffuf / arjun | ID 遍历、参数名与参数值枚举 | |
| LinkFinder / JSFinder | 从 JS 抽接口路由 | 另见 `js-reverse-guide.md` |

---

## 前端线索：文案即路标

页面出现 **「权限不足」「禁止访问」「不允许操作」「需要付费解锁」** 等文案时：

→ **大概率只是前端 JS 拦了，后端没做二次校验。**

打法：
1. F12 删掉前端限制 JS 代码
2. 地址栏**直接输功能对应的接口路由**，跳过页面校验
3. 抓包**删掉前端校验参数**后重放
...
| 链 | 路径 |
|---|---|
| 越权 + 文件下载 | 替换文件 ID → 下载他人合同/证件照 |
| **越权 + XSS（经典提权链）** | 低权限给管理员发带 payload 的消息（消息接口没校验角色）→ 管理员查看 → 偷 Cookie → 进后台 |
| 越权 + 文件上传 | 低权限访问上传接口 → webshell → RCE |
| 越权 + SQLi | 越权进管理后台 → 后台搜索框注入 → 拖库 |

**报告价值**：单独的越权可能是中危，串成链往往能上高危/严重。**发现越权先想它能接什么。**
...
| 语言 | 搜什么 |
|---|---|
| Python Flask | `@login_required` 但**缺角色装饰器**；`request.args.get('userId')` 取身份 |
| Java Spring | 缺 `@PreAuthorize`；`@RequestMapping("/admin/")` 无权限注解 |
| PHP | `$_GET['id']` / `$_POST['user_id']` / `$_COOKIE['role']` |

统一模式：**从请求参数取 userId 的（而不是从 session 取）= 高危信号。**

---

## 测试流程

> 上面是原理与系统化打法，下面是执行步骤。两者对着用。
...
**组件/中间件未授权（Redis、Jenkins、Nacos…）见 `middleware-unauth-test.md`；跳转后仍执行见 `redirect-ear-unauth.md`。本节只讲业务接口。**
...
未授权与越权的判定流程是同一套（找接口 → 换/删凭证 → 差分），区别只在**起手有没有凭证**。

#### 5.2 接口清单从哪来（未授权能不能出，90% 取决于这一步）

按性价比排序：

1. **前端 JS / chunk**：SPA 的接口路径明文写在 JS 里。注意**只用插件（FindSomething 类）拿到的不完整**，必须手工补（懒加载 chunk、按路由拆的包、旧版本 chunk）→ 见 `js-reverse-guide.md`
2. **历史流量 / Burp history / 爬虫**：过往请求里的隐藏接口
3. **同目录衍生 FUZZ**（最省事）：已知 `/api/user/info`，枚举同路径下的
   `/api/user/list` `/api/user/delete` `/api/user/admin` `/api/user/export`
   —— 增删改查往往只给 info 加了鉴权，**同目录的兄弟接口是漏网重灾区**
4. **目录爆破**：dirsearch / ffuf 扫后台路径（配合 `artifact-intel-guide.md` §G 的假阳性过滤）
5. **三维度字典爆破**：参数名 / 参数值（ID、页码、类型值）/ 路由路径

#### 5.3 批量打法

```
1. 收集全量接口清单（上面 5 个来源合并去重）
2. 全部「无凭证」重放一遍 → 看哪些还返回 200 + 有数据
3. 再用低权限凭证重放一遍 → 与高权限响应做差分
4. 命中的逐个人工确认（去伪见下）
```

**去伪（必看，否则全误报）**：
- **统一 404 页返 200** → 与随机不存在路径的响应比长度/内容
- **WAF 拦截页返 200** → 搜「访问禁止 / forbidden / 拦截」关键词
- **SPA catch-all 返 200** → 内容 = 首页，md5 对比
- 这三类假阳性详见 `artifact-intel-guide.md` §3.1

#### 5.4 未授权的四个高发位置

| 位置 | 为什么 |
|---|---|
| **导出 / 下载接口** | `export` `download` `batch` 常被认为"内部用"而漏鉴权 |
| **调试 / 测试接口** | `/debug` `/test` `/api/v1/` 旧版本、上线没关 |
| **回调 / 通知接口** | 设计时假定"只有内部服务会调"，对外也开着 |
| **移动端 / 小程序的独立 API** | 与主站鉴权不是同一套，常是两套鉴权只做了一套 |
