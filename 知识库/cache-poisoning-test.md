...
>
> 写不写只认 `~/.workbuddy-ai/skills/lpp_edu_src/rules/vuln-report-format.md`。只探到缓存键、没有投毒/偷会话 → 继续跟投毒/偷会话。有会话个人页/账单/`/api/me` 就可以打后缀，不必先看见 `X-Cache`。没这类页，不要为了勾表空加 `.css`。

## 一、原有知识库

# 缓存投毒测试手册

> **实战案例（双向引用）** → `csrf-cases.md` §8 多步操作链（**与缓存直接相关的两条完整 getshell**：CatfishCMS 4.5.7「评论插 XSS → 诱管理员 → 自动 CSRF 写文章 → 清缓存 → 访问前端重新生成缓存 → 缓存 PHP 包含 payload」；PHPOK 5.5「CSRF 改 `api_code` → 已知 key 伪造 token → 反序列化 `cache` 类 `__destruct` 写文件」）。**注**：ima 案例库 1604 份中**无缓存投毒独立报告**，本类靠打法支撑，别指望案例索引。
