...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无原型链污染报告**，本篇靠打法支撑。相邻可参考 `js-reverse-guide.md`（在 JS 里定位 `merge`/`extend`/递归赋值等污染点）、`code-audit-cases.md` §1.3（前端 JS / REST 审计思路）。
...
3. Identify technology stack:
   ...
   └── Unknown → 逐个试本文件 Section 1.1~1.5 的 gadget
...
│   │   │   └── Unknown → try each gadget in Sections 1.1–1.5 of this file
│   │   │
...
│   │       └── Match framework against the gadget list in Section 1
│   │
│   └── CLIENT-SIDE
│       ├── jQuery loaded?
