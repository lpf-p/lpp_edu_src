...
> **实战案例（双向引用）** → `attack-chain-cases.md` §2.2（`*.js.map` + reverse-sourcemap 还原源码、`.ds_store` + **DS_Walk** 还原目录）；`infoleak-cases.md` §三（`.git/config`、`.svn/entries`、`/build.xml`、`.env`、`/WEB-INF/web.xml`；`build.xml` 直接拿 SSH 部署账号）+ §1（芋道按 JS 路径回溯 GitHub 源码）；`code-audit-cases.md` 案例 #21（宇视 `main-cgi` `cmd:255` 下 Config.xml 拿账号密码）；`other-census.md` §6.2（`[攻防实战]爽文一把梭` 靠 `oa.zip` 源码泄露）；`archive-inventory.md` §1.1（`15-谈一谈github泄露.pdf`，科普级，留档未精读）。
