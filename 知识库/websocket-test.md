...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无 WebSocket 报告**，本篇靠打法支撑。相邻：`mobile-cases.md` §6（抓包失败与协议分析排查、`protobuf-inspector`/`msgpack-tools` 解二进制帧思路同源）；`xss-cases.md` #1 与 #16（WS 消息进 DOM 时的 XSS 落点）；`idor-test.md`（WS 帧里同样能改对象 ID / 用户标识，鉴权缺陷与 HTTP 同源，按越权写）。
