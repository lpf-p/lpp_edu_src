...
> **实战案例（双向引用）** → `cmd-injection-cases.md` §一 组件指纹速查表（Shiro `rememberMe` 硬编码 key、rome `ToStringBean` gadget）+ §二打法分类 3（`CommonsBeanutils1 + TomcatEcho`、**源码泄露得 key**）；`attack-chain-cases.md` §3（Shiro 默认 key `kPH+bIxk5D2deZiIxcaaaA==`、`shiro_tool.jar` 探 key、**Yso 需重编译**；案例 #22 XStream/Fastjson/Tomcat 不出网）；`code-audit-cases.md` §1.1 + §二危险函数表（`ObjectInputStream.readObject` / `JSON.parseObject(fastjson)`）；`csrf-cases.md` 案例 #18（PHPOK 5.5 CSRF + 反序列化 getshell）。
