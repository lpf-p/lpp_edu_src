...
> **实战案例（双向引用）** → `logic-web-cases.md` **§3**（ima 88 份报告里提炼的并发打法）：成立条件 §3.1、Turbo Intruder 标准流程 §3.2、**脚本三种变体 §3.2.1 双 gate / §3.2.2 参数调优经验值 / §3.2.3 带随机参数时的并发规避**、高价值并发点清单 §3.3、**并发失败的 4 个原因 §3.4**、把竞态绑到钱上提档 §3.5。本篇给工具与原理，那边给"哪类功能点真能中 + 厂商怎么定级"。
