...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无请求走私独立报告**（13 个类型目录里没有该类型），本篇靠打法支撑。相邻：`ghost-bits-cast-test.md`（Java `char`(16bit) → 8bit 字节窄化，走私与协议边界的编码基础）、`attack-chain-cases.md` §4（不出网 / 边界突破时的协议层手法）、`http2-attacks-test.md`（几乎不交，走私走本篇）。
