...
> **实战案例（双向引用）** → 邮件侧是该技巧的主要落点：`logic-web-cases.md` §10.3（系统代发邮件内容篡改：导出接口请求体含 `nickname`/`email`/`headUrl`/`templateId`，改 `headUrl` 注入恶意站点而**发件人仍显示企业官方**）；`xss-cases.md` #20（通达 OA 邮件/问答处存储 XSS，`<img src=x onerror=eval(atob(...))>`）；`weak-password-cases.md` §一（上海交大 SOGo 邮件系统出厂默认 `system:system` / `admin:moohoo`）。相邻：`http-smuggling-test.md`（同为 8bit 协议边界）、`crlf-injection-test.md`。
