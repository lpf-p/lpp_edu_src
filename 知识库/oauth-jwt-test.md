...
> **实战案例（双向引用）** → `vendor-system-cases.md` #21/22（**美团 passport OAuth 账号劫持**：`passport.meituan.com/account/callback/tencent?code=` 可复用；状态「已忽略·内部已知」，说明平台已知难拿赏金）；`ima-case-corpus.md` §2.4（OAuth2.0 绑定劫持：authorize → 微博回调带 `code` → **站点 callback 第三段 302** 是下手点）；`csrf-cases.md` §4（某厂商 OAuth2.0 微博绑定劫持，`callback?state={"can_transfer":true}&code=`）；`code-audit-cases.md` 案例 #11/#12（JWT `alg:none` 置空签名、弱密钥 `hashcat -m 16500` / `jwt_tool` 爆破）；`infoleak-cases.md` §2（`/jeecg-boot/actuator` 的 `httptrace` 留存含 JWT 的登录 cookie）；`attack-chain-cases.md` 案例 #9（token 伪造 → 任意文件读 → o2oa RCE）+ §3（**Nacos token 伪造** `POST /nacos/v1/auth/users/login`）；`mobile-cases.md` §5（登录态与 token 缺陷）。
