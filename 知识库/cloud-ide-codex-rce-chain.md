...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无云 IDE / Codex 类报告**（素材年代早于该类产品普及），本篇靠打法支撑。相邻可参考 `cmd-injection-cases.md` §二打法分类 5「未授权 RCE」（Nacos 2.3.2/2.4.0 配置接口、K8s 8080 未鉴权 `exec`、WebLogic `_async`/`wls-wsat`）——同为「默认口暴露 → 未授权 → RCE」形态；`middleware-unauth-test.md`（未授权面清单）。
