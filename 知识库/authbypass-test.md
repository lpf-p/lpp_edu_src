...
> **实战案例（双向引用）** → `code-audit-cases.md` §1.1（`web.ignoring().antMatchers("/actuator/**")` 把监控页放行导致未授权；`VerifyCodeFilter` 校验逻辑被注释、仅判 `captcha` 非空即放行）；`logic-web-cases.md` §5 验证码 8 种姿势 / §6 密码重置 / §7.3 SSO 与统一认证绕过；`edusrc-cases.md` §一（统一身份认证旁路、教务系统挂 `:18703`/`:8088` 高位端口）；`ima-case-corpus.md` §1（铜陵职院：验证码回显 → 任意密码重置）；`weak-password-cases.md` §一（教育侧初始口令常为身份证后六位）。
...
**找回/申诉整条流程**（状态机、验证码零校验、身份字段逐个独立性、token 四项、定级与佐证合并）见 `password-reset-test.md`。限流与验证码缺陷见 `ratelimit-abuse-test.md`。
**访问后台被跳登录页、但抓包能看见后台内容**（3xx 截断跳转 / CWE-698 重定向后未 return，跳转后操作真执行）见 `redirect-ear-unauth.md`——先判状态码是 3xx 还是 200，两类根因不同，别混。
