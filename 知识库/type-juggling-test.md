...
> **实战案例（双向引用）** → ima 案例库 1604 份中**无 PHP 弱类型 / 类型杂耍报告**，本篇靠打法支撑。相邻：`sqli-cases.md` §五（教育侧万能密码 `'or 1=1--`——那是**注入**不是类型杂耍，别混）、`authbypass-test.md`（认证绕过里的等值/比较缺陷）、`code-audit-cases.md` §1.2（PHP 侧危险函数与比较逻辑）。
