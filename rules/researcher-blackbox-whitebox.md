# 黑盒 SRC + 白盒 0day 审计主流程

> 本文件承载白盒 Phase 0～6、输出规范、能力面与推进节奏。
> 黑盒进站打法认 `dig-scope-workflow.md` §4；挖什么认 `src-value-hunting.md`；浏览器工具认 `playwright-browser-mcp.md`。
...
不要从第一行读到最后一行。先按下面顺序圈定范围：

| # | 优先看什么 | 为什么 |
|---|-----------|--------|
| 1 | 认证 / 授权 / 反序列化 / 模板渲染入口 | 一旦破即接管或 RCE |
| 2 | 解析不可信输入的模块（反序列化 / 模板 / 表达式） | 不可信输入最密集 |
