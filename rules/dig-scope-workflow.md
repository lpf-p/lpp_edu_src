...
> **多资产批量（用户授权「全部资产 / 模式库全量过一遍」、或收到别人给的 N 份报告 zip）：先读 `rules/batch-verify-discipline.md`** —— 基线采样 + 只认结构性证据两条铁律，以及「N 份 = 1 漏洞 × M 主机」的鉴定口径。批量跑出「0 新增」是正常结果，不要为凑数放宽判据。
...
**锁面必建：资产账本（2026-09-18 补 · 硬）**

锁面（用户给了资产清单 / 域名清单）时，**只建种子队列不够** —— 种子队列回答"我搜过哪些方向"，**不回答"清单上这 78 台各自什么下场"**。因此锁面必须并建：

```
资产/资产账本.md     每台一行：状态码 / sha1(体) / CNAME 形态 / 判定 / 依据 / 已做测试
```

- `判定` 列只允许四值：`确证存在` / `已被边缘登记·无后端` / `不可判定` / `确证未配置`（判定方法见 `rules/asset-existence-and-coverage.md` §2）。
- **收工判据 = 该列无空缺**；且**"存在"与"测过"分两列记**（存在 ≠ 测过）。
- 没有这张表，就**说不出"到底测完了没有"**，于是既不敢收工也升不了链 —— 这是"测不深"的收工层原因。

详见 `rules/asset-existence-and-coverage.md` §6「收工三联」。
...
**第 -1 步 · 先判这台存不存在（2026-09-18 补 · 锁面有资产清单时必做 · 详见 `rules/asset-existence-and-coverage.md`）**

**为什么放最前面：** 厂商公布的资产清单里**相当一部分是空壳 / 默认页 / 未部署 / 泛记录**。不先判，你会把力气花在**不存在的资产**上（"测半天没洞"的真实原因常是**这台根本没后端**），同时**漏掉**藏在 404 / 502 后面的真站点。

```
curl -s -D - -o body "https://<host>/"        # 候选
curl -s -o base_host "https://zzz-<16位随机>.<根域>/"   # 主机级基线（绝不可能存在）
curl -s -o base_path "https://<host>/__definitely_not_exist_abc123/"  # 路径级基线
sha1sum body base_host base_path
```

**判定（`rules/asset-existence-and-coverage.md` §2 双轨四象限）：**

| `sha1(body)` | 结论 | 下一步 |
|---|---|---|
| ≠ 两条基线 | ✅ **确证存在**（哪怕状态码是 404/403/502） | **进站**走本节后续 + §4.1/§4.2 |
| = 主机级基线 | 未登记 / 走默认 vhost | 记账本 → `不可判定`，**不进站** |
| = 路径级基线 | 走默认源站 | 记账本 → 存在性不确定，**不进站** |
| TLS 握手失败 | 无站点证书 | 记账本 → `确证未配置` |

**三条硬纪律：**
1. **状态码完全不可信** —— 403 ≠ 受保护、404 ≠ 不存在、200 ≠ 有内容（软 404）。只认响应体。
2. **基线必须同批实时取**（禁止硬编码历史哈希、禁止跨天）。
3. **判不出就写"不可判定"，禁止当真资产往上报**（泛记录 ≠ 不存在，也 ≠ 存在）。

**账本：** 每台主机在这一步就要落 `资产/资产账本.md`（`判定` 列只允许四值，收工前不许有空缺）。详见该文件 §6。

> **跳过本步的唯一情形：** 目标只有单台明确业务站（用户直接给了能打开的 URL），不存在"清单里混空壳"的问题。此时直接进第 0 步。

---

**第 0 步 · 先认系统（2026-09-16 补 · 教育/政务资产必做）**

**厂商成品系统**（国产 OA / 教务 / 统一认证 / 报表…）和**自研系统**的进站顺序是**反的**，别搞混：

| 资产类型 | 第一步 | 为什么 |
|---|---|---|
| **厂商成品系统** | **探根 → 认指纹 → 路由厂商模块**，之后才谈业务 | 业务逻辑是厂商定的，你答不出「钱权状态字段」也不影响开打 —— 打法是查厂商 Nday / 默认口 / 未授权路径，不是逆业务 |
| **自研系统** | 直接走下面的四问 | 业务是你方设计的，四问能答出来，且越权/逻辑是主战场 |

**认系统只取四样**（一次请求就够，别开扫描器）：

```
curl -s -D - -o /dev/null "https://<host>/"     # 看 Server / Set-Cookie / Location
curl -s "https://<host>/" | grep -oE "<title>[^<]*</title>"
curl -s "https://<host>/" | grep -oE 'src="[^"]+"' | head
```
...
| 见到 | 判为 | 处置 |
|---|---|---|
| **`412 Precondition Failed`** + 正文含 `$_ts` + 随机名 cookie | **瑞数 Botgate 动态防护**（国产） | 停。**别发绕过载荷**（不是规则型 WAF，§2 那套编码/HPP 全无效）。先判值不值得啃：目标是逻辑漏洞就上无头浏览器正常走流程；目标就是破防护 → 按 15～20 分钟无苗头跳。详见 `知识库/waf-bypass.md` §9 |
| `Server` 被抹成**字面量** `none` / `Server` / `*****` / 一堆空格 | 有网关或防护介入（**不是「无 Server 头」**） | 继续，但**别把 Server 头当判据**；转行为指纹法 |
| `403` + 中文「温馨提示」类自定义页 | 有 WAF / 访问控制 | 记下阻断页字样（攒样本用），换路径或换手法，别硬刚 |

**判不出系统**：看语言 + 年代，走该文件 §1.3 末尾的兜底；仍判不出再按自研系统四问走。

> 实测依据（2026-09-16，**两轮共 258 个教育资产**）：第一轮 4 个 → 4/4 靠 `Set-Cookie` + `X-Powered-By` + `Location` + `title` 认出；第二轮 24 个 + 第三轮 234 个批量 → **泛微系占多数（e-cology + 移动端）、正方、金智 CAS、瑞数/华为云 WAF 等防护、以及大量自研 SPA**。**结论：厂商成品系统占比过半，指纹表命中率约 45~54%；漏认项全部因缺国产指纹（泛微移动端 / 瑞数 / 华为云 WAF），已补。四问在这些站上答不出来，但完全不影响开打。**
>
> ⚠️ **样本偏差提醒**：这批资产取自某漏洞报告集合，该集合本身泛微偏置（202 份里 124 份打的是同一条泛微路径）。**「泛微占多数」只能说明「在这个集合里」，不能当教育行业市占率引用。** 写报告/下结论时别把样本构成当市场数据。
...
### 4.1.4 功能点过 12 维（反查 · 补矩阵的漏 · 2026-09-18 补）

> **为什么还要这一节：** §4.2 全类型矩阵是**正查** —— 手里有类型清单，去站上找对应入口。它有结构性漏：**清单上没写的形态，你想不起来测**。本节是**反查** —— 对着一个功能点问 12 个问题，**自己命威胁名**，再回头对矩阵。两者正交：正查保证已知类型都碰过，反查保证不漏已知之外。
> **不加负担：** 只对**有业务逻辑的功能点**（登录 / 找回、下单 / 支付、审批 / 流转、上传 / 导出、权限 / 角色、报表）过一遍，静态页跳过。一个功能点 3～5 分钟，结果写进 `{名}_dig/dig-state.json`（见 `desktop-task-folder`）。

**12 维（逐条自问；按功能点实际答，别照着漏洞清单对号）：**

| 维 | 自问 |
|----|------|
| 数据流 | 数据从哪进、经哪些处理、到哪出？有没有地方让输入**原样**进库 / 进命令 / 进模板？ |
| 权限边界 | 这个接口校验调用者角色吗？校验做在哪一层（网关 / 中间件 / 每个接口 / 前端）？能绕吗？ |
| 资源归属 | 对象 id 是数字 / 有序吗？后端校验「这个 id 属不属于当前用户」吗？能枚举到别人的吗？ |
| 状态变更 | 这个操作改什么状态？转换有前置检查吗？能跳步直达终态吗？能从终态回退吗？ |
| 客户端可控值 | 金额 / 数量 / 折扣 / 角色 / 回调 URL 这些「本应服务端定的值」，客户端能传进来覆盖吗？ |
| 并发场景 | 能并发执行吗？同一资源的并发写会不会重复生效（重复领取 / 超卖 / 双花）？ |
| 输出展示 | 输入最终渲染到哪个页面的哪个元素？`innerHTML` 还是 `textContent`？返回的 HTML 字段转义了吗？ |
| 认证与会话 | 登录 / 注册 / 找回有可枚举的差异响应吗？token 能伪造 / 重放吗？ |
| 服务端请求 | 接口收 URL / 主机名 / IP 吗？收后服务端会发请求吗？限协议 / 限 IP 段吗？ |
| 注入面 | 输入进 SQL / 命令 / 模板 / XML / JSON 路径吗？转义够吗？有二次处理让转义失效吗？ |
| 文件操作 | 上传：类型校验在哪、什么方式、能绕吗？路径可控吗？下载 / 删除：路径参数能穿越吗？ |
| 业务逻辑 | 价格 / 数量能为负或 0 吗？券 / 积分 / 抽奖能重复用吗？签名 key 是公开还是私密？状态机能逆向吗？ |

**命出来的威胁项要带推理，禁止只写类型名：**

```
资源归属·订单详情换 id      推理：/order/detail?id= 是自增数字，未见归属校验
客户端可控值·下单改价        推理：下单请求体带 price 字段，前端可改
并发场景·优惠券重复核销      推理：核销接口无幂等键，未见锁
```

⚠️ **只写「IDOR」「越权」这种泛名的，等于没建模** —— 认不出「打哪、凭什么打的」。名字必须带场景。

**收口：每个功能点答三问**（输入面 / 行为面 / 深度面，原文见 `verdict-states.md` §三），写进 `dig-state.json` 的 `coverage_note`。**答不出就是还没测完。**

**与矩阵的接法：** 12 维命出的威胁项 → 逐条对 §4.2 矩阵选探针 → 打；**命出但矩阵里没有对应类型的，照打不误**（那正是反查的价值）。每条打完只能落进 `verdict-states.md` 的五个状态词之一。
...
| **SSRF** | 所有 URL/回调/webhook/file/fetch/import/preview/proxy 类参数：**`127.0.0.1` / 内网单端口 / 外带 DNSLog 三者之一**证明可发起请求即停；看是否跟请求、回显、打到自己的监听。**不再打云元数据**（`169.254.169.254` 拿临时凭证 = 实质接管云资源，已列为硬禁，见 `rules/test-scope-boundary.md` §五） | 清单与抓包里 **零** URL 拉取/代理类字段 |
...
12. **有业务逻辑的功能点过了 §4.1.4 的 12 维**，且每个功能点的 `coverage_note` 三问已答（写进 `dig-state.json`）
13. **每条测过的威胁项落到 `verdict-states.md` 的五个状态词之一** —— 只有 `confirmed` 会被写进报告正文，其余（`suspicious` / `blocked` / `needs`）进盲区清单；标 `not_vulnerable` 的**必须填了 `unruled_out`**。挂着一堆「疑似」不收工
...
- [ ] **有业务逻辑的功能点过 §4.1.4 的 12 维了吗？** 命出的威胁项是带场景的名字还是泛名（「IDOR」这种等于没建模）？`coverage_note` 三问答了吗？  
- [ ] **测过的威胁项是否都落到五个状态词之一？** 报告里还有「疑似 / 可能 / 应该是」吗？标 `not_vulnerable` 的是否都填了 `unruled_out`？（`verdict-states`）  
...
**锁面先过存在性闸（§4.0 第 -1 步）：** 逐台「响应体 vs 同批实时双基线」判存不存在（**状态码不可信**：403/404/200 都会骗人）→ 只对**确证存在**的走全矩阵 → 逐台落 `资产/资产账本.md`；判不出的如实写「不可判定」，**禁止当真资产往上报**。**收工 = 资产账本判定列无空缺 + 种子队列 pending = 0**（`asset-existence-and-coverage` §6）。  
...
**有业务逻辑的功能点另过 §4.1.4 的 12 维（反查）**：正查保证已知类型都碰过，反查保证清单外的不漏；命出的威胁项每条都要落状态词。  
**判定收口（`verdict-states`）：** 一个点测完只有五种说法 —— `confirmed` / `suspicious` / `not_vulnerable` / `blocked` / `needs`；**只有 `confirmed` 进报告正文**，其余进盲区清单；**标 `not_vulnerable` 必须先写 `unruled_out`**（还没排除什么），**不确定就别说安全**。  
